    ...
    <Card class="p-3">
      <div v-if="directories.length > 0" class="flex flex-col gap-4">
        ...
        >
          <a
            href="#"
            @click.prevent="openDirectory(directory.path)"
            class="flex cursor-pointer items-center justify-between overflow-hidden rounded-lg border border-gray-200 p-4 no-underline text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-700"
          >
            <div class="flex items-center gap-2">
              <svg
                class="mr-1 h-5 w-5 align-middle"
                enable-background="new 0 0 347.479 347.479"
                version="1.1"
        ...
        <div v-for="(file, index) in files" class="w-full" :key="index">
          ...
          >
            ...
            <div class="flex gap-4">
              <span title="Rename Log">
                <span @click.prevent="renameLog(file.path)">
                  <svg
                    class="h-5 w-5 align-middle"
                    xmlns="http://www.w3.org/2000/svg"
                    fill="currentColor"
...
export default {
  ...
  methods: {
    currentDirectoryPath() {
      return new URLSearchParams(window.location.search).get("directory_path");
    },
    openDirectory(path) {
      let url = new URL(window.location.href);

      if (path) {
        url.searchParams.set("directory_path", path);
      } else {
        url.searchParams.delete("directory_path");
      }

      // Push a new entry so browser back/forward can navigate directories.
      window.history.pushState({}, "", url);
      this.getFiles(path);
      window.scrollTo(0, 0);
    },
    getFiles(path) {
      let route = "/nova-vendor/laravel-bugster/log-files";
    ...
    renameLog(filePath) {
      ...
      Nova.request()
        .post("/nova-vendor/laravel-bugster/log-files/rename", {
          file_path: filePath
        })
        .then(() => {
          let path = null;
    ...
    handlePopState() {
      this.getFiles(this.currentDirectoryPath());
    }
  },
  mounted() {
    let path = null;
